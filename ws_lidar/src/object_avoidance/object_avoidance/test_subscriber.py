...
class CmdVelSubscriber(Node):
    ...
    def listener_callback(self, msg):
        ...
        angular_z = msg.angular.z  # Steering angle request from publisher
        ...
        # Ensure steering angle remains within limits
        steering_angle = max(15, min(35, steering_angle))

        self.servo.set_servo_pwm('0', steering_angle)
        self.get_logger().info(f"Steering Angle: {steering_angle}°")

        if linear_x > 0:
            self.motor.setMotorModel(0, 2000, 0, 2000)  # Move forward
        elif linear_x < 0:
            self.motor.setMotorModel(0, -2000, 0, -2000)  # Move backward
        else:
