...
class CmdVelSubscriber(Node):
    def __init__(self):
        ...
        # Initialize PCA9685, Motor, and Servo
        self.pwm = PCA9685(0x40)
        self.motor = Motor()
        self.servo = Servo()

        # Subscribe to /cmd_vel for speed control
        ...
        # Subscribe to /steering_angle for exact steering control
        ...
        self.timeout_seconds = 1.0  # Stop if no message for 1 sec
        self.timer = self.create_timer(0.1, self.check_timeout)
        self.get_logger().info("Listening for /cmd_vel and /steering_angle messages...")

        self.steering_angle = 25  # Start at 25° (straight)

    def listener_callback(self, msg):
        """Handles incoming /cmd_vel messages for speed control."""
        self.last_received_time = self.get_clock().now()
        linear_x = msg.linear.x  # Forward/backward speed

        if linear_x > 0:
            # Move forward
            self.motor.setMotorModel(0, 1000, 0, 1000)
        elif linear_x < 0:
            # Move backward
            self.motor.setMotorModel(0, -1000, 0, -1000)
        else:
        ...
        self.get_logger().info(f"Motor Speed: {linear_x}")

    def steering_callback(self, msg):
        """Handles incoming /steering_angle messages for steering control."""
        self.steering_angle = msg.data
        # Directly set the servo to the received angle on channel '0'
        self.servo.set_servo_pwm('0', self.steering_angle)
        self.get_logger().info(f"Steering Angle Set: {self.steering_angle}°")

    def check_timeout(self):
        """Stops motors if no command is received within the timeout period."""
        time_since_last = (self.get_clock().now() - self.last_received_time).nanoseconds / 1e9
