...
class GPSPublisher(Node):
    def __init__(self):
        ...
        self.publisher = self.create_publisher(msg_type=String,topic='GPS_fix', qos_profile=10)
        timer_period=1
        ...
        try:
            ...
            while True:
                ...
                if data.startswith("$GNRMC"):
                    ...
                    lat = original_msg.latitude
                    lon = original_msg.longitude
                    lat = str(lat)
                    lon = str(lon)
                    latlon = lat + "," + lon
                    msg.data = latlon
...
def main(args=None):
    ros.init(args=args)
    gps = GPSPublisher()
    #if latitude > #add some latitude here
    ros.spin(gps)
    ros.shutdown()
